    ...
    return (
      <>
        <div className="wrapper bg-gray-200 bg-opacity-50">
          ...
          <p className="text-center font-Rubik pt-12 md:pt-16 text-2xl md:text-4xl">FAQs
</p>
            {data.map((item, i) => (
                <div className="ss">
              <div className="item py-2" key={i}>
                <div className="title" onClick={() => toggle(i)}>
                  <h2 className="text-lg text-xl md:text-xl text-green-600 font-semibold mb-2 font-Rubik">{item.question}</h2>
                  <span>{selected === i ? '-' : '+'}</span>
                </div>
                <div className={selected === i ? 'content show' : 'content'}>
                  <div className="">
                    <p className="text-slate-600 text-sm md:text-base font-Rubik custom-padding" dangerouslySetInnerHTML={{ __html: item.answer }}></p>
                  </div>
                </div>
              </div>
              </div>
...
const data = [
    ...
    },
    {
      ...
      answer: " Using PrithWe's carbon footprint calculator is easy! Simply log in, select whether you are a household or a business, and fill in the required details about your energy consumption, transportation habits, and other activities. The calculator will then provide you with an estimate of your carbon emissions and it shows interactive charts from which you can take steps for reducing your environmental impact.",
    },
    {
